        ...
        <div style={{ width: '46px', flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
          <div style={{ flex: 1, position: 'relative', borderRight: '1px solid #2a2a35', borderBottom: '1px solid #2a2a35', overflow: 'visible' }}>
            {yLabels.map((v, i) => {
              ...
              return (
                <div key={i} style={{ position: 'absolute', left: 0, right: 0, top: `${topPct}%`, transform: 'translateY(-50%)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: '10px', color: '#999', lineHeight: 1 }}>{v >= 1000000 ? `$${(v/1000000).toFixed(1)}M` : v >= 1000 ? `$${(v/1000).toFixed(0)}k` : `$${v}`}</span>
                  <div style={{ width: '4px', height: '1px', background: '#333' }} />
                </div>
              )
            })}
            {/* Grey start value on Y-axis */}
            {startLineY !== null && (
              <div style={{ position: 'absolute', left: 0, right: 0, top: `${startLineY}%`, transform: 'translateY(-50%)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div style={{ width: '4px', height: '1px', background: '#888' }} />
              </div>
            )}
          </div>
