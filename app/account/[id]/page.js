...
export default function AccountPage() {
  ...
  return (
    <div style={{ height: '100vh', background: '#0a0a0f', overflow: activeTab === 'trades' ? 'hidden' : 'auto' }} onMouseMove={e => setMousePos({ x: e.clientX, y: e.clientY })}>
      ...
      {!isMobile && (
        <div style={{ position: 'fixed', top: '57px', left: '180px', right: 0, zIndex: 46, padding: '22px 24px 18px 24px', background: '#0a0a0f', borderBottom: '1px solid #333', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      {!isMobile && (
        <div style={{ position: 'fixed', top: '57px', left: 0, bottom: 0, width: '180px', padding: '16px 12px', background: '#0a0a0f', zIndex: 45, display: 'flex', flexDirection: 'column', paddingTop: '28px', borderRight: '1px solid #1a1a22' }}>
        <div>
          {['trades', 'statistics', 'notes'].map((tab) => (
            <button
              key={tab}
