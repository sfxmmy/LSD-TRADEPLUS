...
export default function AccountPage() {
  ...
  const showTooltipDelayed = (text, x, y) => {
    ...
    tooltipTimerRef.current = setTimeout(() => {
      // Check if tooltip would go off screen at top, if so show below
  ...
  return (
    <div style={{ height: '100vh', background: '#0a0a0f', overflow: activeTab === 'trades' ? 'hidden' : 'auto' }} onMouseMove={e => setMousePos({ x: e.clientX, y: e.clientY })}>
      ...
      <style>{`
        ...
        select { transition: transform 0.15s ease, box-shadow 0.15s ease; }
        select:hover { transform: scale(1.02); box-shadow: 0 0 8px rgba(255,255,255,0.2) !important; }
      `}</style>
      {/* Global Tooltip */}
      ...
      {!isMobile && (
        <div style={{ position: 'fixed', top: '68px', left: '180px', right: 0, zIndex: 46, height: '56px', boxSizing: 'border-box', padding: '0 12px', background: '#0a0a0f', borderBottom: '1px solid #1a1a22', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flex: 1, minWidth: 0 }}>
            {(() => {
              const getTitle = () => {
                // Notes tab is always just "Notes"
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
              ...
              {activeTab === 'trades' && (
                <button onClick={() => setShowEditInputs(true)} style={{ height: '36px', margin: 0, padding: '0 20px', background: 'transparent', border: '1px solid #2a2a35', borderRadius: '6px', color: '#fff', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1, transition: 'all 0.2s' }} onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; e.currentTarget.style.borderColor = '#3a3a45'; const rect = e.currentTarget.getBoundingClientRect(); showTooltipDelayed('Customize table columns', rect.left + rect.width / 2, rect.top) }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = '#2a2a35'; hideTooltip() }}>Edit Columns</button>
              )}
              ...
              {!viewingSelectedStats && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <button
                    ...
                    onMouseLeave={e => { if (viewMode !== 'this') { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = '#1a1a22'; e.currentTarget.style.color = '#666' } }}
                  >
                  ...
                  {sameTypeAccounts.length > 1 && (
                    <>
                      <button
                        ...
                        onMouseEnter={e => { if (viewMode !== 'all') { e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; e.currentTarget.style.borderColor = '#2a2a35'; e.currentTarget.style.color = '#888' } }}
                        onMouseLeave={e => { if (viewMode !== 'all') { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = '#1a1a22'; e.currentTarget.style.color = '#666' } }}
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      ...
                      <div style={{ position: 'relative' }} data-journal-dropdown>
                        <button
                          ...
                          onMouseEnter={e => { if (viewMode !== 'selected') { e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; e.currentTarget.style.borderColor = '#2a2a35'; e.currentTarget.style.color = '#888' } }}
                          onMouseLeave={e => { if (viewMode !== 'selected') { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = '#1a1a22'; e.currentTarget.style.color = '#666' } }}
                        >
                    ...
                    <tr>
                      ...
                      {enabledInputs.map((inp, i) => (
                        <th
                          ...
                          style={{
                            padding: isMobile ? '5px 8px 9px 8px' : '5px 12px 9px 12px',
                            textAlign: 'center',
                      ...
                      <th style={{ padding: '5px 12px 9px 12px', textAlign: 'center', color: '#999', fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', borderBottom: '1px solid #1a1a22', background: '#0a0a0f', minWidth: '70px' }}>Placed</th>
                      <th style={{ padding: '5px 12px 9px 12px', textAlign: 'center', color: '#999', fontSize: '12px', fontWeight: 600, borderBottom: '1px solid #1a1a22', background: '#0a0a0f', minWidth: '70px' }}></th>
                    </tr>
                  </thead>
                  <tbody>
            ...
            <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setViewingTrade(null)}>
              <div style={{ background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '16px', padding: '0', width: '95%', maxWidth: '1000px', maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ padding: '16px 20px', borderBottom: '1px solid #1a1a22', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <button onClick={() => { setViewingTrade(null); startEditTrade(trade) }} style={{ padding: '6px 14px', background: 'transparent', border: '1px solid #2a2a35', borderRadius: '6px', color: '#fff', fontWeight: 600, fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', transition: 'all 0.2s' }} onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; e.currentTarget.style.borderColor = '#3a3a45' }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = '#2a2a35' }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                      Edit
                    </button>
                    <button onClick={() => { setViewingTrade(null); setDeleteConfirmId(trade.id) }} style={{ padding: '6px 14px', background: 'transparent', border: '1px solid #2a2a35', borderRadius: '6px', color: '#ef4444', fontWeight: 600, fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', transition: 'all 0.2s' }} onMouseEnter={e => { e.currentTarget.style.background = 'rgba(239,68,68,0.1)'; e.currentTarget.style.borderColor = '#ef4444' }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = '#2a2a35' }}>
                    ...
                    <button onClick={() => setViewingTrade(null)} style={{ width: '32px', height: '32px', background: 'transparent', border: '1px solid #2a2a35', borderRadius: '6px', color: '#666', fontSize: '18px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }} onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; e.currentTarget.style.borderColor = '#3a3a45' }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = '#2a2a35' }}>×</button>
                  </div>
                </div>
            ...
            <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: '12px', marginBottom: '12px', alignItems: 'stretch' }}>
              ...
              <div style={{ width: isMobile ? '100%' : '200px', background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '12px', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #1a1a22' }}>
                  ...
                  <span style={{ fontSize: '13px', fontWeight: 700, color: positiveColor }}>+${displayAvgWin}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #1a1a22' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #1a1a22' }}>
                  ...
                  <span style={{ fontSize: '13px', fontWeight: 700, color: positiveColor }}>{displayStreaks.mw}</span>
                </div>
                  ...
                  {(() => {
                    ...
                    if (sorted.length >= 2) {
                      if (equityCurveGroupBy === 'total') {
                        ...
                        lines = [{ name: 'Total', points, color: positiveColor }]
                      } else {
                        const groups = {}
                        sorted.forEach(t => {
                          let key
...
{/* Line labels moved to right-side column to avoid overlap */}
                          ...
                          <div style={{ flex: 1, display: 'flex' }}>
                            ...
                            <div style={{ flex: 1, position: 'relative', borderLeft: '1px solid #2a2a35', borderBottom: '1px solid #2a2a35' }}>
                              ...
                              <div style={{ position: 'absolute', inset: '0', display: 'flex', alignItems: 'flex-end', gap: '6px', padding: '0 4px' }}>
                                {entries.map((item, i) => {
                                  const hPct = item.val === 0 ? 0 : Math.max((Math.abs(item.val) / niceMax) * 100, 5)
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '12px', alignItems: 'flex-start' }}>
              ...
              <div style={{ background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '10px 14px' }}>
                ...
                {(() => { const dayNames = ['M', 'T', 'W', 'T', 'F']; const dayPnL = [0, 0, 0, 0, 0]; displayTrades.forEach(t => { const day = new Date(t.date).getDay(); if (day >= 1 && day <= 5) dayPnL[day - 1] += parseFloat(t.pnl) || 0 }); const maxAbs = Math.max(...dayPnL.map(p => Math.abs(p)), 1); return (<div style={{ display: 'flex', justifyContent: 'space-between', gap: '6px' }}>{dayPnL.map((pnl, i) => { const heightPct = Math.max(Math.min((Math.abs(pnl) / maxAbs) * 100, 100), 8); const isPositive = pnl >= 0; const color = isPositive ? positiveColor : '#ef4444'; return (<div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}><div style={{ fontSize: '10px', fontWeight: 600, color: pnl === 0 ? '#444' : color }}>{pnl !== 0 ? (pnl >= 0 ? '+' : '') + Math.round(pnl) : '0'}</div><div style={{ width: '100%', height: '24px', background: '#1a1a22', borderRadius: '3px', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'flex-end' }}><div style={{ width: '100%', height: pnl === 0 ? '2px' : `${heightPct}%`, background: pnl === 0 ? '#2a2a35' : color, borderRadius: '2px', opacity: pnl === 0 ? 1 : 0.8 }} /></div><div style={{ fontSize: '9px', fontWeight: 600, color: '#666' }}>{dayNames[i]}</div></div>) })}</div>) })()}
              </div>
              {/* AI Insight */}
      ...
      {buttonTooltip && (
        <div style={{
          ...
          {buttonTooltip.text}
          {/* Arrow - points up if showing below, points down if showing above */}
          <div style={{
            position: 'absolute',
