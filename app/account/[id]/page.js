...
export default function AccountPage() {
  ...
  return (
    <div style={{ height: '100vh', background: '#0a0a0f', overflow: activeTab === 'trades' ? 'hidden' : 'auto' }} onMouseMove={e => setMousePos({ x: e.clientX, y: e.clientY })}>
      ...
      {!isMobile && (
        <div style={{ position: 'fixed', top: '69px', left: '180px', right: 0, zIndex: 46, padding: '11px 40px', background: '#0a0a0f', borderBottom: '1px solid #1a1a22', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', height: '38px' }}>
            <span style={{ fontSize: '28px', fontWeight: 700, color: '#fff', lineHeight: '38px' }}>{account?.name}</span>
          </div>
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center', height: '38px' }}>
            ...
            {activeTab === 'trades' && selectMode && (
              <>
                <span style={{ fontSize: '12px', color: '#22c55e', fontWeight: 600, lineHeight: 1 }}>{selectedTrades.size} selected</span>
                <button onClick={() => { const allSelected = filteredTrades.every(t => selectedTrades.has(t.id)); if (allSelected) { const newSet = new Set(selectedTrades); filteredTrades.forEach(t => newSet.delete(t.id)); setSelectedTrades(newSet) } else { const newSet = new Set(selectedTrades); filteredTrades.forEach(t => newSet.add(t.id)); setSelectedTrades(newSet) } }} style={{ height: '38px', padding: '0 16px', background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.4)', borderRadius: '8px', color: '#22c55e', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{filteredTrades.every(t => selectedTrades.has(t.id)) && filteredTrades.length > 0 ? 'Deselect All' : 'Select All'}</button>
                {selectedTrades.size > 0 && <button onClick={() => { setViewingSelectedStats(true); setActiveTab('statistics') }} style={{ height: '38px', padding: '0 16px', background: 'rgba(34,197,94,0.15)', border: '1px solid #22c55e', borderRadius: '8px', color: '#22c55e', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>View Stats</button>}
                {selectedTrades.size > 0 && <button onClick={() => setDeleteSelectedConfirm(true)} style={{ height: '38px', padding: '0 16px', background: 'rgba(239,68,68,0.15)', border: '1px solid #ef4444', borderRadius: '8px', color: '#ef4444', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Delete</button>}
                <button onClick={exitSelectMode} style={{ height: '38px', padding: '0 16px', background: '#1a1a22', border: '1px solid #2a2a35', borderRadius: '8px', color: '#666', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Cancel</button>
              </>
            )}
            {!selectMode && (
              <button onClick={() => { setDraftFilters({...filters, quickSelect: ''}); setShowFilters(true) }} style={{ height: '38px', padding: '0 16px', background: hasActiveFilters ? 'rgba(34,197,94,0.15)' : 'transparent', border: hasActiveFilters ? '1px solid #22c55e' : '1px solid #2a2a35', borderRadius: '6px', color: hasActiveFilters ? '#22c55e' : '#fff', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <button type="button" onClick={() => setTradeForm({...tradeForm, outcome: 'be'})} style={{ flex: 1, padding: '9px', background: tradeForm.outcome === 'be' ? 'rgba(255,255,255,0.1)' : '#0a0a0f', border: `2px solid ${tradeForm.outcome === 'be' ? '#888' : '#1a1a22'}`, borderRadius: '8px', color: tradeForm.outcome === 'be' ? '#888' : '#666', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>BE</button>
                </div>
              </div>
            </div>
