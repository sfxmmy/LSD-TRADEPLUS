...
export default function AccountPage() {
  ...
  return (
    <div style={{ height: '100vh', background: '#0a0a0f', overflow: activeTab === 'trades' ? 'hidden' : 'auto' }} onMouseMove={e => setMousePos({ x: e.clientX, y: e.clientY })}>
      ...
      {!isMobile && (
        <div style={{ position: 'fixed', top: '53px', left: '180px', right: 0, zIndex: 46, padding: '12px 40px', background: '#0a0a0f', borderBottom: '1px solid #1a1a22', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      {!isMobile && (
        <div style={{ position: 'fixed', top: '53px', left: 0, bottom: 0, width: '180px', padding: '16px 12px 12px 12px', background: '#0a0a0f', zIndex: 45, display: 'flex', flexDirection: 'column', borderRight: '1px solid #1a1a22' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {['trades', 'statistics', 'notes'].map((tab) => (
            <button
              key={tab}
      ...
      {/* MAIN CONTENT */}
      <div style={{ marginLeft: isMobile ? 0 : '180px', marginTop: isMobile ? '100px' : '127px', padding: isMobile ? '12px' : '0' }}>

        {/* TRADES TAB */}
        {activeTab === 'trades' && (
          <div style={{ position: 'relative', height: 'calc(100vh - 127px)' }}>
        ...
        {activeTab === 'statistics' && (() => {
          ...
          return (
          <div style={{ padding: isMobile ? '0' : '8px 12px' }}>
