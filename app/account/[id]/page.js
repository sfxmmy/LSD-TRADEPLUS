...
export default function AccountPage() {
  ...
  function addNewInput() { setInputs([...inputs, { id: `custom_${Date.now()}`, label: 'New Field', type: 'text', required: false, enabled: true, fixed: false, options: [] }]) }
  ...
  // Get custom select inputs for dropdowns
  function getCustomSelectInputs() {
    return inputs.filter(i => i.type === 'select' && i.enabled && !['outcome'].includes(i.id))
  }

  // Get truly custom inputs (not in defaults)
  function getTrulyCustomInputs() {
    return inputs.filter(i => !defaultInputs.find(d => d.id === i.id) && i.enabled)
  }
  ...
  const nonFixedDefaultInputs = enabledInputs.filter(i => !i.fixed && !['symbol', 'outcome', 'pnl', 'riskPercent', 'rr', 'date'].includes(i.id) && defaultInputs.find(d => d.id === i.id))
  const trulyCustomInputs = getTrulyCustomInputs()
  ...
  const tabDescriptions = {
    ...
    statistics: 'Detailed statistics and charts.',
    notes: 'Keep notes about your trading.'
  }

  // Equity Curve Component with proper axes and green dot
  function EquityCurve() {
    ...
    const pts = [{ y: cum, date: 'Start', pnl: 0, idx: 0 }]
    sorted.forEach((t, i) => { cum += parseFloat(t.pnl) || 0; pts.push({ y: cum, date: t.date, pnl: parseFloat(t.pnl), symbol: t.symbol, idx: i + 1 }) })
    
    const minY = Math.min(...pts.map(p => p.y))
    const maxY = Math.max(...pts.map(p => p.y))
    const yRange = maxY - minY || 1
    const yStep = Math.ceil(yRange / 4 / 1000) * 1000 || 1000
    const yMin = Math.floor(minY / yStep) * yStep
    const yMax = Math.ceil(maxY / yStep) * yStep
    const actualRange = yMax - yMin || yStep
    ...
    if (pts.length > 1) {
      ...
      if (first) xLabels.push({ label: new Date(first).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit' }), pct: 0 })
      if (last && last !== first) xLabels.push({ label: new Date(last).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit' }), pct: 100 })
    }
    ...
    function handleMouseMove(e) {
      ...
      const xPos = (pt.idx / (pts.length - 1)) * 100
      const yPos = 100 - ((pt.y - yMin) / actualRange) * 100
      setHoverPoint({ ...pt, xPos, yPos })
    }
    
    return (
      <div style={{ height: '100%', display: 'flex' }}>
        <div style={{ width: '55px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', paddingBottom: '20px', paddingRight: '8px' }}>
          {yLabels.map((v, i) => <span key={i} style={{ fontSize: '11px', color: '#666', textAlign: 'right' }}>${(v/1000).toFixed(0)}k</span>)}
        </div>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ flex: 1, position: 'relative', borderLeft: '1px solid #333', borderBottom: '1px solid #333' }}>
            <svg ref={svgRef} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }} viewBox="0 0 100 100" preserveAspectRatio="none" onMouseMove={handleMouseMove} onMouseLeave={() => setHoverPoint(null)}>
              ...
              <polygon points={pts.map((p, i) => `${(i / (pts.length - 1)) * 100},${100 - ((p.y - yMin) / actualRange) * 100}`).join(' ') + ' 100,100 0,100'} fill="url(#eqG)" />
              <polyline points={pts.map((p, i) => `${(i / (pts.length - 1)) * 100},${100 - ((p.y - yMin) / actualRange) * 100}`).join(' ')} fill="none" stroke="#22c55e" strokeWidth="2" vectorEffect="non-scaling-stroke" />
            </svg>
            {hoverPoint && (
              <>
                <div style={{ position: 'absolute', left: `${hoverPoint.xPos}%`, top: `${hoverPoint.yPos}%`, transform: 'translate(-50%, -50%)', width: '12px', height: '12px', borderRadius: '50%', background: '#22c55e', border: '2px solid #fff', pointerEvents: 'none', zIndex: 10 }} />
                <div style={{ position: 'absolute', left: `${hoverPoint.xPos}%`, top: `${hoverPoint.yPos}%`, transform: `translate(${hoverPoint.xPos > 80 ? '-110%' : '15px'}, -50%)`, background: '#1a1a22', border: '1px solid #2a2a35', borderRadius: '6px', padding: '8px 12px', pointerEvents: 'none', zIndex: 10, whiteSpace: 'nowrap' }}>
                  <div style={{ fontSize: '11px', color: '#888' }}>{hoverPoint.date === 'Start' ? 'Start' : new Date(hoverPoint.date).toLocaleDateString()}</div>
                  <div style={{ fontSize: '14px', fontWeight: 700, color: '#fff' }}>${hoverPoint.y.toLocaleString()}</div>
                  {hoverPoint.symbol && <div style={{ fontSize: '12px', color: hoverPoint.pnl >= 0 ? '#22c55e' : '#ef4444' }}>{hoverPoint.symbol}: {hoverPoint.pnl >= 0 ? '+' : ''}${hoverPoint.pnl}</div>}
                </div>
              </>
            )}
          </div>
          <div style={{ height: '20px', display: 'flex', justifyContent: 'space-between', paddingTop: '4px' }}>
            {xLabels.map((l, i) => <span key={i} style={{ fontSize: '11px', color: '#666' }}>{l.label}</span>)}
          </div>
        </div>
      </div>
    )
  ...
  // Bar Chart Component with proper axes
  function BarChart() {
    ...
    trades.forEach(t => {
      ...
      else key = getExtraData(t)[graphGroupBy]
      if (!key) return
    ...
    return (
      <div style={{ height: '100%', display: 'flex' }}>
        <div style={{ width: '50px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', paddingBottom: '20px', paddingRight: '8px' }}>
          {yLabels.map((l, i) => <span key={i} style={{ fontSize: '11px', color: '#666', textAlign: 'right' }}>{l}</span>)}
        </div>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ flex: 1, display: 'flex', alignItems: 'flex-end', gap: '6px', borderLeft: '1px solid #333', borderBottom: '1px solid #333', padding: '0 4px 4px 4px' }}>
            {entries.map((item, i) => {
              const hPct = Math.max((Math.abs(item.val) / maxVal) * 100, 5)
              const isGreen = barGraphMetric === 'winrate' ? item.val >= 50 : item.val >= 0
              return (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%', justifyContent: 'flex-end' }} onMouseEnter={() => setHoverIdx(i)} onMouseLeave={() => setHoverIdx(null)}>
                  {hoverIdx === i && <div style={{ fontSize: '11px', color: isGreen ? '#22c55e' : '#ef4444', marginBottom: '4px', fontWeight: 700 }}>{item.disp}</div>}
                  <div style={{ width: '100%', height: `${hPct}%`, background: isGreen ? '#22c55e' : '#ef4444', borderRadius: '2px 2px 0 0', opacity: hoverIdx === null || hoverIdx === i ? 1 : 0.5 }} />
                </div>
              )
            })}
          </div>
          <div style={{ height: '20px', display: 'flex', gap: '6px', paddingTop: '4px' }}>
            {entries.map((item, i) => <div key={i} style={{ flex: 1, textAlign: 'center', fontSize: '10px', color: '#666', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.name}</div>)}
          </div>
        </div>
      </div>
    )
  ...
  // Net Daily PnL Component with proper axes
  function NetDailyPnL() {
    const [hoverIdx, setHoverIdx] = useState(null)
    
    if (dailyPnL.length === 0) return <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#666' }}>No data</div>
    
    const maxAbs = Math.max(...dailyPnL.map(d => Math.abs(d.pnl)), 1)
    const yLabels = [`+$${Math.round(maxAbs)}`, '$0', `-$${Math.round(maxAbs)}`]
    
    return (
      <div style={{ height: '100%', display: 'flex' }}>
        <div style={{ width: '55px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', paddingRight: '8px' }}>
          {yLabels.map((l, i) => <span key={i} style={{ fontSize: '11px', color: '#666', textAlign: 'right' }}>{l}</span>)}
        </div>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '3px', borderLeft: '1px solid #333', position: 'relative' }}>
            <div style={{ position: 'absolute', left: 0, right: 0, top: '50%', height: '1px', background: '#333' }} />
            {dailyPnL.map((d, i) => {
              const hPct = (Math.abs(d.pnl) / maxAbs) * 50
              return (
                <div key={i} style={{ flex: 1, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', position: 'relative' }} onMouseEnter={() => setHoverIdx(i)} onMouseLeave={() => setHoverIdx(null)}>
                  {d.pnl >= 0 ? (
                    <div style={{ position: 'absolute', bottom: '50%', left: '10%', right: '10%', height: `${hPct}%`, background: '#22c55e', borderRadius: '2px 2px 0 0' }} />
                  ) : (
                    <div style={{ position: 'absolute', top: '50%', left: '10%', right: '10%', height: `${hPct}%`, background: '#ef4444', borderRadius: '0 0 2px 2px' }} />
                  )}
                  {hoverIdx === i && (
                    <div style={{ position: 'absolute', top: d.pnl >= 0 ? '10px' : 'auto', bottom: d.pnl < 0 ? '10px' : 'auto', left: '50%', transform: 'translateX(-50%)', background: '#1a1a22', border: '1px solid #2a2a35', borderRadius: '4px', padding: '6px 10px', zIndex: 10, whiteSpace: 'nowrap' }}>
                      <div style={{ fontSize: '10px', color: '#888' }}>{new Date(d.date).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit' })}</div>
                      <div style={{ fontSize: '13px', fontWeight: 700, color: d.pnl >= 0 ? '#22c55e' : '#ef4444' }}>{d.pnl >= 0 ? '+' : ''}${d.pnl.toFixed(0)}</div>
                    </div>
                  )}
                </div>
              )
  ...
  // Generate stats for custom inputs
  function CustomInputStats() {
    const customSelectInputs = inputs.filter(i => i.type === 'select' && !defaultInputs.find(d => d.id === i.id) && i.enabled)
    if (customSelectInputs.length === 0) return null
    
    return (
      <div style={{ marginTop: '16px' }}>
        <div style={{ background: '#f59e0b', color: '#000', padding: '8px 12px', borderRadius: '6px 6px 0 0', fontSize: '12px', fontWeight: 600 }}>⚡ CUSTOM INPUT STATS</div>
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
          {customSelectInputs.map(inp => {
            const grouped = {}
            trades.forEach(t => {
              const val = getExtraData(t)[inp.id]
              if (!val) return
              if (!grouped[val]) grouped[val] = { w: 0, l: 0, pnl: 0, count: 0 }
              grouped[val].count++
              grouped[val].pnl += parseFloat(t.pnl) || 0
              if (t.outcome === 'win') grouped[val].w++
              else if (t.outcome === 'loss') grouped[val].l++
            })
            
            const entries = Object.entries(grouped).map(([name, d]) => ({
              name,
              wr: d.w + d.l > 0 ? Math.round((d.w / (d.w + d.l)) * 100) : 0,
              avgPnl: d.count > 0 ? Math.round(d.pnl / d.count) : 0,
              count: d.count
            ...
            return (
              <div key={inp.id} style={{ flex: '1 1 300px', background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '0 0 8px 8px', padding: '16px' }}>
                <div style={{ fontSize: '13px', color: '#f59e0b', fontWeight: 600, marginBottom: '12px', textTransform: 'uppercase' }}>{inp.label} Breakdown</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {entries.slice(0, 5).map((e, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', background: '#0a0a0e', borderRadius: '4px' }}>
                      <span style={{ fontSize: '13px', color: '#fff' }}>{e.name}</span>
                      <div style={{ display: 'flex', gap: '12px' }}>
                        <span style={{ fontSize: '12px', color: e.wr >= 50 ? '#22c55e' : '#ef4444' }}>{e.wr}% WR</span>
                        <span style={{ fontSize: '12px', color: e.avgPnl >= 0 ? '#22c55e' : '#ef4444' }}>{e.avgPnl >= 0 ? '+' : ''}${e.avgPnl}/trade</span>
                        <span style={{ fontSize: '11px', color: '#666' }}>{e.count} trades</span>
                      </div>
                    </div>
                  ))}
                </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f' }}>
      ...
      <div style={{ position: 'fixed', top: '57px', left: '180px', right: 0, zIndex: 40, padding: '14px 24px', background: '#0a0a0f', borderBottom: '1px solid #1a1a22', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          {activeTab === 'trades' && <button onClick={() => setShowEditInputs(true)} style={{ padding: '12px 24px', background: 'transparent', border: '1px solid #2a2a35', borderRadius: '8px', color: '#fff', fontSize: '14px', cursor: 'pointer' }}>Edit Columns</button>}
      ...
      <div style={{ position: 'fixed', top: '57px', left: 0, bottom: 0, width: '180px', padding: '16px 12px', borderRight: '1px solid #1a1a22', background: '#0a0a0f', zIndex: 45, display: 'flex', flexDirection: 'column' }}>
        <div>
          {['trades', 'statistics', 'notes'].map((tab) => (
            <button key={tab} onClick={() => { setActiveTab(tab); if (tab === 'statistics') setHasNewInputs(false) }} style={{ width: '100%', padding: '16px 20px', marginBottom: '8px', background: activeTab === tab ? '#22c55e' : 'transparent', border: activeTab === tab ? 'none' : '1px solid #2a2a35', borderRadius: '8px', color: activeTab === tab ? '#fff' : '#888', fontSize: '14px', fontWeight: 600, textTransform: 'uppercase', cursor: 'pointer', textAlign: 'center' }}>{tab}</button>
          ))}
        </div>
        <div style={{ marginTop: 'auto' }}>
          {hasNewInputs && (
            <div style={{ padding: '10px 12px', background: '#f59e0b', borderRadius: '6px', marginBottom: '10px', fontSize: '11px', color: '#000', fontWeight: 600 }}>⚡ New stats available for custom inputs!</div>
          )}
      ...
      <div style={{ marginLeft: '180px', marginTop: '124px', padding: '16px 24px', minHeight: 'calc(100vh - 124px)' }}>
        ...
        {activeTab === 'trades' && (
          <div style={{ background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', overflow: 'hidden' }}>
            {trades.length === 0 ? (
              <div style={{ padding: '60px', textAlign: 'center', color: '#888' }}>No trades yet. Click "+ LOG NEW TRADE"</div>
            ) : (
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: '#0a0a0e' }}>
                      <th style={{ padding: '14px 12px', textAlign: 'center', color: '#888', fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', borderBottom: '1px solid #1a1a22' }}>Symbol</th>
                      <th style={{ padding: '14px 12px', textAlign: 'center', color: '#888', fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', borderBottom: '1px solid #1a1a22' }}>W/L</th>
                      <th style={{ padding: '14px 12px', textAlign: 'center', color: '#888', fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', borderBottom: '1px solid #1a1a22' }}>PnL</th>
                      <th style={{ padding: '14px 12px', textAlign: 'center', color: '#888', fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', borderBottom: '1px solid #1a1a22' }}>%</th>
                      <th style={{ padding: '14px 12px', textAlign: 'center', color: '#888', fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', borderBottom: '1px solid #1a1a22' }}>RR</th>
                      <th style={{ padding: '14px 12px', textAlign: 'center', color: '#888', fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', borderBottom: '1px solid #1a1a22' }}>Date</th>
                      <th style={{ padding: '14px 12px', textAlign: 'center', color: '#888', fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', borderBottom: '1px solid #1a1a22' }}></th>
                    </tr>
                  </thead>
                  <tbody>
                    {trades.map((trade) => {
                      ...
                      const hasExtras = nonFixedDefaultInputs.length > 0 || trulyCustomInputs.length > 0
                      return (
                        <>
                          <tr key={trade.id} style={{ borderBottom: hasExtras ? 'none' : '1px solid #141418' }}>
                            <td style={{ padding: '12px', fontWeight: 600, fontSize: '15px', textAlign: 'center', color: '#fff' }}>{trade.symbol}</td>
                            <td style={{ padding: '12px', textAlign: 'center' }}>
                              <span style={{ padding: '4px 10px', borderRadius: '4px', fontSize: '12px', fontWeight: 600, background: trade.outcome === 'win' ? 'rgba(34,197,94,0.15)' : trade.outcome === 'loss' ? 'rgba(239,68,68,0.15)' : 'rgba(255,255,255,0.1)', color: trade.outcome === 'win' ? '#22c55e' : trade.outcome === 'loss' ? '#ef4444' : '#888' }}>
                                {trade.outcome === 'win' ? 'WIN' : trade.outcome === 'loss' ? 'LOSS' : 'BE'}
                              </span>
                            </td>
                            <td style={{ padding: '12px', textAlign: 'center', fontWeight: 600, fontSize: '15px', color: pnlValue >= 0 ? '#22c55e' : '#ef4444' }}>{pnlValue >= 0 ? '+' : ''}${pnlValue.toFixed(0)}</td>
                            ...
                            <td style={{ padding: '12px', textAlign: 'center', fontSize: '13px', color: '#888' }}>{new Date(trade.date).toLocaleDateString()}</td>
                            <td style={{ padding: '12px', textAlign: 'center' }}><button onClick={() => deleteTrade(trade.id)} style={{ background: 'transparent', border: 'none', color: '#666', cursor: 'pointer', fontSize: '18px' }}>×</button></td>
                          </tr>
                          {hasExtras && (
                            <tr key={`${trade.id}-extra`} style={{ borderBottom: '1px solid #141418', background: '#0a0a0e' }}>
                              <td colSpan={7} style={{ padding: '8px 12px' }}>
                                <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', fontSize: '12px' }}>
                                  {nonFixedDefaultInputs.map(inp => {
                                    const val = inp.id === 'direction' ? trade.direction : inp.id === 'notes' ? trade.notes : extra[inp.id]
                                    if (!val && inp.id !== 'rating') return null
                                    return (
                                      <span key={inp.id} style={{ color: '#666' }}>
                                        <span style={{ color: '#888' }}>{inp.label}:</span>{' '}
                                        {inp.type === 'rating' ? (
                                          <span>{[1,2,3,4,5].map(i => <span key={i} style={{ color: i <= parseInt(val || 0) ? '#22c55e' : '#333' }}>★</span>)}</span>
                                        ) : inp.id === 'notes' && val ? (
                                          <button onClick={() => setShowExpandedNote(val)} style={{ background: 'transparent', border: 'none', color: '#22c55e', cursor: 'pointer', fontSize: '12px' }}>View</button>
                                        ) : inp.id === 'image' && val ? (
                                          <button onClick={() => setShowExpandedImage(val)} style={{ background: 'transparent', border: 'none', color: '#22c55e', cursor: 'pointer', fontSize: '12px' }}>View</button>
                                        ) : (
                                          <span style={{ color: inp.id === 'direction' ? (val === 'long' ? '#22c55e' : '#ef4444') : inp.id === 'confidence' && val === 'High' ? '#22c55e' : inp.id === 'confidence' && val === 'Low' ? '#ef4444' : '#fff' }}>{val}</span>
                                        )}
                                      </span>
                                    )
                                  })}
                                  {trulyCustomInputs.map(inp => {
                                    const val = extra[inp.id]
                                    if (!val) return null
                                    return <span key={inp.id} style={{ color: '#666' }}><span style={{ color: '#f59e0b' }}>{inp.label}:</span> <span style={{ color: '#fff' }}>{val}</span></span>
                                  })}
                                </div>
                              </td>
                            </tr>
        ...
        {activeTab === 'statistics' && (
          <>
              ...
              <div style={{ width: '200px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {[
                  ...
                  <div key={i} style={{ background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '6px', padding: '10px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: '16px', fontWeight: 700, color: s.c }}>{s.v}</span>
                  </div>
                ))}
                <div style={{ background: '#0d0d12', border: '2px solid #22c55e', borderRadius: '6px', padding: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '4px' }}>
                  <span style={{ fontSize: '12px', color: '#888' }}>Total Trades</span>
                  <span style={{ fontSize: '24px', fontWeight: 700, color: '#22c55e' }}>{trades.length}</span>
                </div>
              </div>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '16px', height: '180px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                    ...
                    <span style={{ fontSize: '12px', color: '#888' }}>Start: ${startingBalance.toLocaleString()} → Current: <span style={{ color: '#22c55e' }}>${currentBalance.toLocaleString()}</span></span>
                  </div>
                  <div style={{ height: 'calc(100% - 30px)' }}><EquityCurve /></div>
                </div>
                <div style={{ background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '16px', height: '180px', display: 'flex', gap: '16px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', justifyContent: 'center' }}>
                    <div style={{ padding: '8px', background: '#0a0a0e', border: '1px solid #1a1a22', borderRadius: '6px' }}>
                      ...
                      <select value={barGraphMetric} onChange={e => setBarGraphMetric(e.target.value)} style={{ width: '100px', padding: '6px', background: '#141418', border: '1px solid #1a1a22', borderRadius: '4px', color: '#fff', fontSize: '12px' }}>
                    ...
                    <div style={{ padding: '8px', background: '#0a0a0e', border: '1px solid #1a1a22', borderRadius: '6px' }}>
                      ...
                      <select value={graphGroupBy} onChange={e => setGraphGroupBy(e.target.value)} style={{ width: '100px', padding: '6px', background: '#141418', border: '1px solid #1a1a22', borderRadius: '4px', color: '#fff', fontSize: '12px' }}>
                        <option value="symbol">Pairs</option>
                        <option value="direction">Direction</option>
                        <option value="session">Session</option>
                        <option value="confidence">Confidence</option>
                        <option value="timeframe">Timeframe</option>
                        {getTrulyCustomInputs().filter(i => i.type === 'select').map(inp => <option key={inp.id} value={inp.id}>{inp.label}</option>)}
                      </select>
                    </div>
                  </div>
                </div>
            ...
            {/* ROW 2: Net Daily PnL + Direction */}
            <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
              <div style={{ flex: 2, background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '16px' }}>
                <div style={{ fontSize: '12px', color: '#888', textTransform: 'uppercase', marginBottom: '10px' }}>Net Daily PnL</div>
                <div style={{ height: '100px' }}><NetDailyPnL /></div>
              </div>
              <div style={{ flex: 1, background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '16px' }}>
                <div style={{ fontSize: '12px', color: '#888', textTransform: 'uppercase', marginBottom: '10px' }}>Direction Split</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                  <span style={{ fontSize: '13px', color: '#22c55e', fontWeight: 600 }}>{longPct}% Long</span>
                  ...
                  <span style={{ fontSize: '13px', color: '#ef4444', fontWeight: 600 }}>{100 - longPct}% Short</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div><span style={{ fontSize: '11px', color: '#666' }}>Avg Rating</span><div style={{ marginTop: '4px' }}>{[1,2,3,4,5].map(i => <span key={i} style={{ color: i <= Math.round(parseFloat(avgRating)) ? '#22c55e' : '#333', fontSize: '14px' }}>★</span>)}</div></div>
                  <div style={{ textAlign: 'right' }}><span style={{ fontSize: '11px', color: '#666' }}>Rating</span><div style={{ fontSize: '18px', fontWeight: 700, color: '#fff' }}>{avgRating}</div></div>
                </div>
              </div>
            </div>

            {/* ROW 3: Stats + Donut + Analysis + Expectancy */}
            <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
              <div style={{ width: '300px', background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '16px', display: 'flex' }}>
                <div style={{ flex: 1 }}>
                  {[{ l: 'Avg. Trend', v: avgTrend }, { l: 'Avg. Rating', v: avgRating + '★' }, { l: 'Avg. PnL', v: (avgPnl >= 0 ? '+' : '') + '$' + avgPnl }, { l: 'Most Traded', v: mostTradedPair }, { l: 'Most Used RR', v: mostUsedRR }, { l: 'Best RR', v: bestRR }].map((item, i) => (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minWidth: '100px' }}>
                  <div style={{ fontSize: '10px', color: '#888', marginBottom: '8px' }}>BEST PAIR</div>
                  {(() => {
                    const ps = {}; trades.forEach(t => { if (!ps[t.symbol]) ps[t.symbol] = { w: 0, l: 0, pnl: 0 }; if (t.outcome === 'win') ps[t.symbol].w++; else if (t.outcome === 'loss') ps[t.symbol].l++; ps[t.symbol].pnl += parseFloat(t.pnl) || 0 })
                    const best = Object.entries(ps).sort((a, b) => b[1].pnl - a[1].pnl)[0]
                    if (!best) return <div style={{ color: '#666' }}>-</div>
                    ...
                    return (<><div style={{ position: 'relative', width: size, height: size }}><svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}><circle cx={size/2} cy={size/2} r={r} fill="none" stroke="#ef4444" strokeWidth={stroke} /><circle cx={size/2} cy={size/2} r={r} fill="none" stroke="#22c55e" strokeWidth={stroke} strokeDasharray={c} strokeDashoffset={c * (1 - wr/100)} strokeLinecap="round" /></svg><div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontSize: '11px', fontWeight: 700, color: '#fff' }}>{best[0]}</div><div style={{ fontSize: '14px', fontWeight: 700, color: '#22c55e' }}>{wr}%</div></div></div><div style={{ display: 'flex', gap: '8px', marginTop: '6px', fontSize: '10px' }}><span><span style={{ color: '#22c55e' }}>●</span> Win</span><span><span style={{ color: '#ef4444' }}>●</span> Loss</span></div></>)
                  })()}
                </div>
              </div>
              ...
              <div style={{ flex: 1, background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '16px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {(() => {
                    const groups = {}
                    trades.forEach(t => {
                      let key = analysisGroupBy === 'direction' ? t.direction?.toUpperCase() : getExtraData(t)[analysisGroupBy]
                      if (!key) return
                      if (!groups[key]) groups[key] = { w: 0, l: 0, pnl: 0, count: 0 }
                      groups[key].count++; groups[key].pnl += parseFloat(t.pnl) || 0
                      if (t.outcome === 'win') groups[key].w++; else if (t.outcome === 'loss') groups[key].l++
                    })
                    return Object.entries(groups).slice(0, 4).map(([name, data]) => {
                      ...
                      return <div key={name} style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: '13px', color: '#fff' }}>{name}:</span><span style={{ fontSize: '14px', fontWeight: 700, color: val >= 0 ? '#22c55e' : '#ef4444' }}>{disp}</span></div>
                    })
                  })()}
                </div>
              </div>
              ...
              <div style={{ width: '120px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ flex: 1, background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '12px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ fontSize: '10px', color: '#888', marginBottom: '4px' }}>AVG LOSS EXP.</div>
                  <div style={{ fontSize: '20px', fontWeight: 700, color: '#ef4444' }}>-${lossExpectancy}</div>
                  <div style={{ fontSize: '9px', color: '#666' }}>per trade</div>
                </div>
                <div style={{ flex: 1, background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '12px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ fontSize: '10px', color: '#888', marginBottom: '4px' }}>EXPECTANCY</div>
                  <div style={{ fontSize: '24px', fontWeight: 700, color: parseFloat(expectancy) >= 0 ? '#22c55e' : '#ef4444' }}>${expectancy}</div>
                  <div style={{ fontSize: '9px', color: '#666' }}>per trade</div>
                </div>
              </div>
            </div>

            {/* ROW 4: Performance (vertical) + Streaks (vertical) + Risk/Progress (horizontal) */}
            <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
              <div style={{ width: '200px', background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '16px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {[{ l: 'Best Day', v: bestDay ? `+$${Math.round(bestDay.pnl)}` : '-', c: '#22c55e' }, { l: 'Worst Day', v: worstDay ? `$${Math.round(worstDay.pnl)}` : '-', c: '#ef4444' }, { l: 'Biggest Win', v: `+$${biggestWin}`, c: '#22c55e' }, { l: 'Biggest Loss', v: `$${biggestLoss}`, c: '#ef4444' }].map((item, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px', background: '#0a0a0e', borderRadius: '4px' }}>
              ...
              <div style={{ width: '200px', background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '16px' }}>
                <div style={{ fontSize: '12px', color: '#888', textTransform: 'uppercase', marginBottom: '10px' }}>Streaks</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {[{ l: 'Max Wins', v: streaks.mw, c: '#22c55e' }, { l: 'Max Losses', v: streaks.ml, c: '#ef4444' }, { l: 'Trading Days', v: tradingDays, c: '#fff' }, { l: 'Trades/Day', v: avgTradesPerDay, c: '#fff' }].map((item, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px', background: '#0a0a0e', borderRadius: '4px' }}>
                      <span style={{ fontSize: '11px', color: '#666' }}>{item.l}</span>
                      <span style={{ fontSize: '14px', fontWeight: 700, color: item.c }}>{item.v}</span>
                    </div>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ flex: 1, background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '16px' }}>
                  <div style={{ fontSize: '12px', color: '#888', textTransform: 'uppercase', marginBottom: '10px' }}>Risk Management</div>
                  <div style={{ display: 'flex', gap: '16px' }}>
                    {[{ l: 'Avg RR', v: avgRR + 'R' }, { l: 'Risk/Reward', v: returnOnRisk + 'x' }, { l: 'Profit Factor', v: profitFactor }].map((item, i) => (
                      <div key={i} style={{ flex: 1, textAlign: 'center', padding: '10px', background: '#0a0a0e', borderRadius: '4px' }}>
                        <div style={{ fontSize: '10px', color: '#666', marginBottom: '4px' }}>{item.l}</div>
                        <div style={{ fontSize: '18px', fontWeight: 700, color: '#fff' }}>{item.v}</div>
                      </div>
                    ))}
                  </div>
                </div>
                <div style={{ flex: 1, background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '16px' }}>
                  <div style={{ fontSize: '12px', color: '#888', textTransform: 'uppercase', marginBottom: '10px' }}>Account Progress</div>
                  <div style={{ display: 'flex', gap: '16px' }}>
                    {[{ l: 'Starting', v: '$' + startingBalance.toLocaleString(), c: '#888' }, { l: 'Current', v: '$' + currentBalance.toLocaleString(), c: '#22c55e' }, { l: 'Growth', v: ((currentBalance / startingBalance - 1) * 100).toFixed(1) + '%', c: currentBalance >= startingBalance ? '#22c55e' : '#ef4444' }].map((item, i) => (
                      <div key={i} style={{ flex: 1, textAlign: 'center', padding: '10px', background: '#0a0a0e', borderRadius: '4px' }}>
                        <div style={{ fontSize: '10px', color: '#666', marginBottom: '4px' }}>{item.l}</div>
          ...
          <>
            ...
            <div style={{ background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '20px', marginBottom: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                <span style={{ fontSize: '12px', color: '#888', textTransform: 'uppercase' }}>Write {notesSubTab} Note</span>
              ...
              <textarea value={noteText} onChange={e => setNoteText(e.target.value)} placeholder={`Write your ${notesSubTab} note...`} style={{ width: '100%', minHeight: '120px', padding: '14px', background: '#0a0a0e', border: '1px solid #1a1a22', borderRadius: '8px', color: '#fff', fontSize: '14px', lineHeight: '1.6', resize: 'vertical', boxSizing: 'border-box' }} />
            ...
            <div style={{ background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '20px' }}>
              <span style={{ fontSize: '12px', color: '#888', textTransform: 'uppercase' }}>{notesSubTab} Notes</span>
              <div style={{ marginTop: '14px', display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '400px', overflowY: 'auto' }}>
                {notesSubTab === 'custom' ? ((notes.custom || []).length === 0 ? <div style={{ padding: '40px', textAlign: 'center', color: '#666' }}>No custom notes yet.</div> : (notes.custom || []).map((note, idx) => (<div key={idx} style={{ padding: '16px', background: '#0a0a0e', borderRadius: '8px', border: '1px solid #1a1a22' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}><span style={{ fontSize: '14px', color: '#22c55e', fontWeight: 600 }}>{note.title}</span><div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}><span style={{ fontSize: '12px', color: '#666' }}>{new Date(note.date).toLocaleDateString()}</span><button onClick={() => deleteNote('custom', idx)} style={{ background: 'transparent', border: 'none', color: '#666', cursor: 'pointer', fontSize: '16px' }}>×</button></div></div><div style={{ fontSize: '14px', color: '#fff', lineHeight: '1.6', whiteSpace: 'pre-wrap' }}>{note.text}</div></div>))) : (Object.keys(notes[notesSubTab] || {}).length === 0 ? <div style={{ padding: '40px', textAlign: 'center', color: '#666' }}>No {notesSubTab} notes yet.</div> : Object.entries(notes[notesSubTab] || {}).sort((a, b) => new Date(b[0]) - new Date(a[0])).map(([date, text]) => (<div key={date} style={{ padding: '16px', background: '#0a0a0e', borderRadius: '8px', border: '1px solid #1a1a22' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}><span style={{ fontSize: '14px', color: '#22c55e', fontWeight: 600 }}>{new Date(date).toLocaleDateString('en-GB', { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric' })}</span><button onClick={() => deleteNote(notesSubTab, date)} style={{ background: 'transparent', border: 'none', color: '#666', cursor: 'pointer', fontSize: '16px' }}>×</button></div><div style={{ fontSize: '14px', color: '#fff', lineHeight: '1.6', whiteSpace: 'pre-wrap' }}>{text}</div></div>)))}
              </div>
            </div>
          </>
        )}
      ...
      {showAddTrade && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }} onClick={() => setShowAddTrade(false)}>
          <div style={{ background: '#0d0d12', border: '1px solid #1a1a22', borderRadius: '8px', padding: '28px', width: '560px', maxHeight: '85vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            {(nonFixedDefaultInputs.length > 0 || trulyCustomInputs.length > 0) && (
              <>
                <div style={{ fontSize: '11px', color: '#666', marginBottom: '10px', textTransform: 'uppercase' }}>Additional Fields</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px', marginBottom: '16px' }}>
                  {[...nonFixedDefaultInputs, ...trulyCustomInputs].map(input => (
                    <div key={input.id} style={{ gridColumn: input.type === 'textarea' ? 'span 2' : 'span 1' }}>
                      <label style={{ display: 'block', fontSize: '11px', color: trulyCustomInputs.find(i => i.id === input.id) ? '#f59e0b' : '#888', marginBottom: '6px', textTransform: 'uppercase' }}>{input.label}</label>
                      {input.type === 'select' ? (
                        <select value={tradeForm[input.id] || ''} onChange={e => setTradeForm({...tradeForm, [input.id]: e.target.value})} style={{ width: '100%', padding: '10px', background: '#0a0a0e', border: '1px solid #1a1a22', borderRadius: '6px', color: '#fff', fontSize: '14px', boxSizing: 'border-box' }}>
                          {input.options?.map(o => <option key={o} value={o.toLowerCase()}>{o}</option>)}
                        </select>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
              {inputs.map((input, i) => (
                <div key={input.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px', background: input.fixed ? '#141418' : '#0a0a0e', borderRadius: '6px', opacity: input.fixed ? 0.7 : 1 }}>
                  ...
                  {input.type === 'select' && !input.fixed && <button onClick={() => openOptionsEditor(i)} style={{ padding: '6px 10px', background: '#22c55e', border: 'none', borderRadius: '6px', color: '#fff', fontSize: '11px', cursor: 'pointer' }}>Opts</button>}
